...
int main(){
    ...
    while(min<max){
        ...
        for (int i=mid;i<numH;i++){
            if (min_needed>needed+maxx(0,2*(shops[numH-1-mid]-maxH))+2*maxH-houses[i])
             min_needed = needed+maxx(0,2*(shops[numH-1-mid]-maxH))+2*maxH-houses[i];
         cout<<needed<<"  "<<2*(shops[numH-1-mid]-maxH)<<"  "<<2*maxH-houses[i]<<"  "<<min_needed<<endl;
            needed+= 2*maxx(0,minn(houses[i+1]-houses[i],shops[i-mid]-houses[i]));
            cout<<houses[i+1]-houses[i]<<"  "<<shops[i-mid]-houses[i]<<"  "<<needed<<endl;
        }
